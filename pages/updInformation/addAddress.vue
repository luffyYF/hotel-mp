	...
	<view class="addAddress">
		...
		<view style="text-align: center;" class="row">
			<radio-group @change="radioChange">
				<label class="radio">
					<radio value="M" :checked="addressDetails.memSex == 'M' ? true : false" color="#06c1ae" />
					先生
				</label>
				<label class="radio">
					<radio value="W" :checked="addressDetails.memSex == 'W' ? true : false" color="#06c1ae" />
					女士
				</label>
			</radio-group>
		</view>
		...
		<button @tap="upd" style="font-size:27.17391rpx;color:white;background-color:#06c1ae;margin:36.23188rpx;">修改</button>
	</view>
</template>
...
export default {
	...
	onLoad(opt) {
		...
		if (JSON.parse(opt.obj).saveType == 'ADDRESS') {
			...
			that.addressDetails.invoicePk = JSON.parse(opt.obj).invoicePk;
		}
	...
	methods: {
		...
		upd() {
			...
			api.invoiceUpd({
				invoicePk: that.addressDetails.invoicePk,
